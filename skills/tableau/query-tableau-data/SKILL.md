...
2. **Authenticate first — run this before anything else.**
   ...
   Do not manually check for `.env` — `SdkConfig` uses a 3-level fallback chain (cwd → skill root → workspace root) and raises immediately if anything is missing. Let it fail loudly. See [AUTH.md](docs/api/AUTH.md) to troubleshoot.
...
└── src/                                  # source code
    ...
    │
    └── query_tableau_datasource/         # Python package (importable as query_tableau_datasource)
